...
export default function DAOPanel({ account, loading, proposals, handleVote, handleExecute, qvCounts, setQvCounts }) {
  ...
  return (
    <>
      <div style={{ fontWeight: 700, fontSize: 24, marginBottom: 24, color: S.text }}>申訴提案</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {proposals.length === 0 ? (
          ...
          return (
            <div key={p.id} style={{ background: S.card, border: `1px solid ${p.executed ? S.border : "#c4b5fd"}`, borderRadius: 16, padding: 28, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ fontSize: 16, color: S.muted, marginBottom: 16 }}>
                  ...
                  {canVote ? (
                    <div>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button onClick={() => handleExecute(p.id)}         disabled={loading} style={mkBtn("#475569", loading)}>結案</button>
                      </div>
                    </div>
                  ) : (
