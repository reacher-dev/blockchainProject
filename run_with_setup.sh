...
find_pico_port() {
  ...
  if [[ -n "${PICO_PORT:-}" && "${PICO_PORT}" != "auto" ]]; then
    printf '%s' "$PICO_PORT"
    return 0
  fi
  ...
  done < <(ls /dev/cu.usbmodem* /dev/tty.usbmodem* /dev/cu.usbserial* /dev/ttyACM* 2>/dev/null || true)
...
if ! PICO_PORT="$(find_pico_port)"; then
  echo "No Pico serial device found. Connect the Pico W and rerun with --port /dev/cu.usbmodemXXX ." >&2
  exit 1
fi
